...
<html>
  ...
  <body>
	    	...
	    	<div id="list_inner1"> 
			    ...
			    <c:forEach items="${pageBean.list }" var="clubarticle">
					...
					<td class="list4 font10">
						<c:if test="${clubarticle.audit==0 }"><span>未审核</span></c:if>
						<c:if test="${clubarticle.audit==1 }"><span>已审核</span></c:if>
						<c:if test="${clubarticle.audit==2 }"><span>审核不通过</span></c:if>
					</td>
					<td class="list5 font10">
						<c:if test="${clubarticle.release==0 }"><span>未发布</span></c:if>
						<c:if test="${clubarticle.release==1 }"><span>已发布</span></c:if>
					</td>
					<td class=list6>
	...
	<div style="width: 380px; margin: 0 auto; margin-top: 50px;">
		...
		<c:if test="${pageBean.currentPage!=1 }">
				<a href="${pageContext.request.contextPath}/draftclubservlet?currentPage=${pageBean.currentPage-1 }&id=4" aria-label="Previous">
					<span aria-hidden="true">&laquo;</span>
				</a>
		</c:if>
		...
		<c:forEach begin="1" end="${pageBean.totalPage }" var="page">
			...
			<c:if test="${page!=pageBean.currentPage }">
				<a href="${pageContext.request.contextPath}/draftclubservlet?currentPage=${page }&id=4">&nbsp;&nbsp;${page }&nbsp;&nbsp;</a>
			</c:if>
		</c:forEach>
		...
		<c:if test="${pageBean.currentPage!=pageBean.totalPage }">
				<a href="${pageContext.request.contextPath}/draftclubservlet?currentPage=${pageBean.currentPage+1 }&id=4" aria-label="Next"> 
					<span aria-hidden="true">&raquo;</span>
				</a>
		</c:if>
