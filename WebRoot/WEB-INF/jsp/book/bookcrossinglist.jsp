...
<html>
  ...
  <body id="top">
	<c:forEach items="${pageBean.list }" var="booklist" varStatus="status">
	<table class="topic border" style="width:100%; border-bottom-color: currentColor; border-bottom-width: medium; border-bottom-style: none; margin-top:10px;" cellSpacing="0" cellPadding="0">
			<tr>
				<th colSpan="5">
					<span>&nbsp;&nbsp;共有<b style="color: rgb(255, 102, 0);">${booklist.totalreply }</b>人留言
					</span>书名:${booklist.bookname }
				</th>
			</tr>
	</table>
	<table class="bbslist border" style="width:100%; table-layout: fixed; word-break: break-all;" cellSpacing="0" cellPadding="0">
			<tr>
				<td class="infoleft leftcolor" vAlign="top" rowSpan="3"><br>
				    <span>发布者:${booklist.createperson }</span>
					<c:forEach items="${alltypelist }" var="typelist">
						<c:if test="${typelist.id==booklist.booktype }"><span>类型：${typelist.typename }</span></c:if>
					</c:forEach> 
					<%-- <span>类型:${alltypelist[secondlist.producttype] }</span> --%>
					<span>状态：<b class="green"> 
						<c:if test="${booklist.state==0 }"><a href="javascript:" onclick="finish(${booklist.id})">未结帖</a></c:if> 
						<c:if test="${booklist.state==1 }">已结帖</c:if>
					</b></span>
					<span >发布时间:<br></span>
					<span style="font-size:15px">${booklist.createtime }</span>
				</td>
				<td style="line-height: 120%;" class="inforight" vAlign="top">
					...
					<table cellSpacing="0" cellPadding="0" style="table-layout: fixed;" >
						<tr>
						    ...
						    <a href="seconddetailservlet?secondid=${book.id }&id=1"><img alt="#" src="${booklist.img }" 
						    onload="javascript:DrawImage(this,300,250)" style="cursor:pointer;"/></a>
						   </div></div></td>
							<td>
								<div style="text-indent: 24px; overflow: hidden; font-size: 12pt; word-break: break-all;
								 word-wrap: break-word;" id="textstyle_1">
									${booklist.content }
			...
			<tr>
				<td class="usermenu2" colspan="1">
					...
					<a href="javascript:" onclick="del(${booklist.id})">删除</a>
					&nbsp;|&nbsp; 
	...
	<div style="width: 380px; margin: 0 auto; margin-top: 20px;">
		...
		<c:if test="${pageBean.currentPage!=1 }">
				<a href="${pageContext.request.contextPath}/booklistservlet?id=0
				&currentPage=${pageBean.currentPage-1 }&articleid=${bArticle.id}" aria-label="Previous">
					<span aria-hidden="true">&laquo;</span>
				</a>
		</c:if>
		...
		<c:forEach begin="1" end="${pageBean.totalPage }" var="page">
			...
			<c:if test="${page!=pageBean.currentPage }">
				<a href="${pageContext.request.contextPath}/booklistservlet?id=0
				&currentPage=${page }&articleid=${bArticle.id}">&nbsp;&nbsp;${page }&nbsp;&nbsp;</a>
			</c:if>
		</c:forEach>
		...
		<c:if test="${pageBean.currentPage!=pageBean.totalPage&&pageBean.totalPage!=0 }">
				<a href="${pageContext.request.contextPath}/booklistservlet?id=0
				&currentPage=${pageBean.currentPage+1 }&articleid=${bArticle.id}" aria-label="Next"> 
					<span aria-hidden="true">&raquo;</span>
				</a>
		</c:if>
...
<script type="text/javascript" language="JavaScript">
	...
	function del(articleId){
		if(confirm("确定要删除吗？")){
			location="${pageContext.request.contextPath}/delbookcrossingservlet?articleId="+articleId;
		}
	}
	
	function finish(articleId){
		if(confirm("确定要结帖吗？")){
			location="${pageContext.request.contextPath}/updatebookstateservlet?articleId="+articleId;
		}
	}
