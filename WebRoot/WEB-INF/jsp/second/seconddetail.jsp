...
<html>
  ...
  <body>
     ...
     <img id="top" alt="图片" src="img/second.jpg" width="100%">
     <div class="nav" style="font-size: 18px;">
	     <div style="float:left;">
			<img src="img/forum_readme.gif" />
			<a href="${pageContext.request.contextPath}/secondlistservlet">交易首页</a>&nbsp;>>&nbsp;详情
		 </div>
	 </div>
			...
			<tr>
				<td class="infoleft leftcolor" vAlign="top" rowSpan="3"><br>
					...
					<span>状态：<b class="green"> 
						<c:if test="${secondhand.state==0&&secondhand.buyorsale==1 }"><a href="javascript:" onclick="finish(${secondhand.id})">未售出</a></c:if> 
						<c:if test="${secondhand.state==1&&secondhand.buyorsale==1 }">已售出</c:if>
						<c:if test="${secondhand.state==0&&secondhand.buyorsale==0 }"><a href="javascript:" onclick="finish(${secondhand.id})">未购买</a></c:if> 
				...
				<td style="line-height: 120%;" class="inforight" vAlign="top">
					...
					<span style="line-height: 20px;" class="font10">
						<c:if test="${secondhand.price!=0 }">Price：￥${secondhand.price }</c:if>
					</span>
					<br/>
