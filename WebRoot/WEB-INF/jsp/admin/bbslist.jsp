	...
	<body>
		...
		<table class="table table-bordered table-hover definewidth m10">
		    ...
		    <c:forEach items="${pageBean.list }" var="article">
				<tr>
					<td><a href="${pageContext.request.contextPath }/bbsdetailArticle?bbstitle=${article.title }&bbscontent=${article.content}&id=1" title="查看所有">${article.title }</a></td>
		...
		<div style="width: 380px; margin: 0 auto; margin-top: 50px;">
			...
			<c:if test="${pageBean.currentPage!=1 }">
					<a href="${pageContext.request.contextPath}/admin/bbslistservlet?currentPage=${pageBean.currentPage-1 }" aria-label="Previous">
						<span aria-hidden="true">&laquo;</span>
					</a>
			</c:if>
			...
			<c:forEach begin="1" end="${pageBean.totalPage }" var="page">
				...
				<c:if test="${page!=pageBean.currentPage }">
					<a href="${pageContext.request.contextPath}/admin/bbslistservlet?currentPage=${page }">&nbsp;&nbsp;${page }&nbsp;&nbsp;</a>
				</c:if>
			</c:forEach>
			...
			<c:if test="${pageBean.currentPage!=pageBean.totalPage }">
					<a href="${pageContext.request.contextPath}/admin/bbslistservlet?currentPage=${pageBean.currentPage+1 }" aria-label="Next"> 
						<span aria-hidden="true">&raquo;</span>
					</a>
			</c:if>
...
function del(articleId){
		if(confirm("确定要删除吗？")){
		location="${pageContext.request.contextPath}/abbsdelArticleservlet?articleId="+articleId;
	}
}
</script>