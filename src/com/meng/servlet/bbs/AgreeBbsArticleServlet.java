...
public class AgreeBbsArticleServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(bbsArtId==null){
			//agree=agree+1
			bbsArticleDao.updateAgree(aticleId);
			bArtIdDao.AddbbsArtIdforagree(stuid,aticleId);
		}
		else{
			//如果没点击过不同意
			if(bbsArtId.getForflag()==1){
			...
			//点击过不同意
				...
				bArtIdDao.updateforFlag2(stuid, aticleId);
				if (bbsArtId.getFlag()==1) {
					//disagree=disagree-1
					bbsArticleDao.updateDisagree1(aticleId);
					//agree=agree+1
